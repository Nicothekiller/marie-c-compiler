...
## Goal
Define and implement a practical 0.1.0 C subset grammar in Pest for `marie-c-compiler`.
...
- Cast expressions `(type)expr` (for now)
 - Inline assembly extension (planned for 0.3.0)
...
- Use declaration-specifier + declarator shape for C-like declarations.
 - Keep parser permissive where needed; semantic rejections and diagnostics come in 0.2.0.
- Parse `else if` naturally via `else` + nested `if` statement.

## 0.2.0 specification

### Goals
 - Keep 0.1.0 syntax stable and add robust diagnostics + semantic validation.
- Expand grammar only where needed for explicit rejection of unsupported C features.

### Parser/grammar scope
- Preserve all 0.1.0 constructs.
 - Add `while (expr) statement` as a supported 0.2.0 iteration statement.
...
- Keep `goto`/labels and inline asm out of 0.2.0 grammar implementation.
...
- Declaration and symbol checks:
 ...
 - Type checks for 0.1.0/0.2.0 subset:
...
- Full C type system.
- Optimizations.
- Full control-flow feature parity with C (only `while` is added beyond 0.1.0 control flow).
...
### Control-flow extensions (optional for 0.3.0)
...
### Data/model extensions (0.3.0)
 - Expand `struct` and array handling beyond 0.1.0 baseline as needed.
- Improve pointer semantics and memory layout rules for backend lowering.

### Backend alignment goals (0.3.0)
 - Ensure all 0.3.0 constructs map cleanly to Marie labels/jumps/data layout.
